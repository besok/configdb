...
static LOG_FILE_NAME: &str = "commit_log.data";
...
impl CommitLog {
    fn create(dir: &Path) -> Result<Self, LogError> {
        if dir.is_file() || !dir.exists() {
            return Err(LogError);
        }

        let mut idx_buf = dir.to_path_buf().clone();
        let mut log_buf = dir.to_path_buf().clone();

        idx_buf.push(INDEX_FILE_NAME);
        log_buf.push(LOG_FILE_NAME);

        let idx = idx_buf.as_path();
        let log = log_buf.as_path();

        File::create(idx)?;
        File::create(log)?;

        Ok(CommitLog { idx: *Path::new(""),log: *Path::new("s") })
}
...
# [test]
fn commit_log_test() {
let result = CommitLog::create(Path::new(r"C:\temp\test_r"));
}
